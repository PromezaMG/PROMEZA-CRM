...
const _getKeyBC = () => {
  ...
  try {
    ...
    _keyBC.onmessage = (e) => {
      if (e && e.data === "req") {
        try { const v = sessionStorage.getItem(SESSION_CRYPTO_KEY); if (v) _keyBC.postMessage({ type: "key", v }); } catch (_) {}
      }
    };
  } catch (_) { _keyBC = null; }
  return _keyBC;
...
const _requestKeyFromSiblings = () => new Promise((resolve) => {
  ...
  let done = false;
  const onMsg = (e) => { if (!done && e && e.data && e.data.type === "key" && e.data.v) { done = true; try { bc.removeEventListener("message", onMsg); } catch (_) {} resolve(e.data.v); } };
  try { bc.addEventListener("message", onMsg); bc.postMessage("req"); } catch (_) { resolve(null); return; }
  setTimeout(() => { if (!done) { done = true; try { bc.removeEventListener("message", onMsg); } catch (_) {} resolve(null); } }, 600);
});
...
const loadSessionKey = async () => {
  ...
  if (!b64) {
    ...
    if (b64) { try { sessionStorage.setItem(SESSION_CRYPTO_KEY, b64); } catch (e) {} }
  }
