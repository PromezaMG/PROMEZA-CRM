        ...
        <div className="actions">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            <div style={{ display: "flex", gap: 4 }}>
                ...
                {showCallMenu && p.phone && (
                  <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, background: "var(--bg)", border: "1px solid var(--line)", borderRadius: 10, boxShadow: "var(--shadow-lg)", zIndex: 200, minWidth: 180, overflow: "hidden" }}>
                    ...
                    <div style={{ padding: 4, display: "flex", flexDirection: "column", gap: 2 }}>
                      {[...new Set([...(p.phones || []).map(ph => ph && ph.value).filter(Boolean), p.phone].filter(Boolean))].map((num, i) => {
                        const lbl = (p.phones || []).find(ph => ph && ph.value === num);
                        return (
                          <div key={i} style={{ display: "flex", gap: 4 }}>
                            <button className="btn" style={{ flex: 1, justifyContent: "flex-start", gap: 8, padding: "9px 12px" }}
                              onClick={() => { window.location.href = "tel:" + num; setShowCallMenu(false); }}>
                              <Icon name="phone" size={14} />
                              <div style={{ textAlign: "left" }}>
                                <div style={{ fontWeight: 600, fontSize: 12.5 }}>{lbl && lbl.label ? lbl.label : (lang === "es" ? "Teléfono" : "Phone")}</div>
                                <div style={{ fontSize: 11, color: "var(--ink-3)", fontFamily: "var(--font-mono)" }}>{num}</div>
                              </div>
                            </button>
                            <a className="btn" title="WhatsApp" href={"https://wa.me/" + waNum(num)} target="_blank" rel="noopener noreferrer" style={{ padding: "0 10px", color: "#25D366", borderColor: "#25D36640", background: "#f0fdf4", display: "inline-flex", alignItems: "center", justifyContent: "center" }}
              ...
              {p.phone && (
                <a className="btn" title="WhatsApp" href={"https://wa.me/" + waNum(p.phone)} target="_blank" rel="noopener noreferrer"
                  style={{ padding: "0 10px", color: "#25D366", borderColor: "#25D36640", background: "#f0fdf4", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
